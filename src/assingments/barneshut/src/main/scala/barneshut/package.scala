...
package object barneshut {
  ...
  case class Fork(
    ...
    val massY: Float = if(mass == 0) centerX else (nw.mass * nw.massY + ne.mass * ne.massY + sw.mass * sw.massY + se.mass * se.massY)/mass
  ...
  extends Quad {
    ...
    def insert(b: Body): Quad =
      if(size < minimumSize)
        ...
        )((acc: Fork, body: Body) => acc.insert(body))
      else
